...
import { readFile, writeFile, successLog, messageLog, isLowerCaseEqual, getPlatformInfo, isTranslationPlatformNotFound, showLanguageList, changeLanguageCode } from "./helpers.js";
...
async function onUse(name) {
    if (await isTranslationPlatformNotFound(name))
        return;
...
async function onSetTranslation(name, { appid, secretKey }) {
    ...
    platform.appid = appid !== null && appid !== void 0 ? appid : platform.appid;
    platform.key = secretKey !== null && secretKey !== void 0 ? secretKey : platform.key;
    await writeFile(configPath, config);
...
export { onList, onUse, onSetTranslation, onTranslate, onSetTranslateLanguage };
